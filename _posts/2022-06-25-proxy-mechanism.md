...
예를 들어, Member class와 Team class가 서로 연관관계 매핑이 되어 있고 비즈니스 로직 상 Member를 참조할 때 Team을 함께 참조하지 않는다고 생각해보자. `em.find(Member.class, ...)` 이런 식으로 Member 객체를 가져오게 된다면 Team 테이블도 함께 데이터베이스에서 가져오지만 사용하지 않는다. 이런 작업이 계속 반복되게 되면 성능에 큰 영향을 주게 될 것이다. 따라서 가짜 객체를 내세워서 역할을 하게 한 뒤, 실제로 진짜 객체 값이 필요할 때 영속성 컨텍스트를 통해 데이터베이스에서 객체를 참조해 값을 주입해주면 되는 것이다.
